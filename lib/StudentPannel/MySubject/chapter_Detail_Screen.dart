// chapter_Detail_Screen.dart (WITH ADDED LOGGING)
...
class _ChapterDetailsScreenState extends State<ChapterDetailsScreen>
  ...
  void initState() {
    ...
    // ✅ NEW: Favorite animation controller
    ...
    // ✅ Get initial favorite status from chapter model
  ...
  Future<void> _syncDailyActivity() async {
    if (_userCode == null) {
      print('⚠️ Cannot sync: userCode is null');
      return;
    }
  ...
  Future<void> _loadMaterials() async {
    ...
    try {
      ...
      final response = await StudentSubjectService.getChapterMaterials(_userCode!, widget.chapter.chapterId);
      ...
      if (mounted) {
        ...
        CustomSnackbar.showError(context, _errorMessage!, title: 'Failed to Load Materials');
      }
    }
  }
  ...
  void initializeTrackingData() {
    ...
    _materialsData!.materials.forEach((materialKey, materialValue) {
      if (materialValue is Map &&
        ...
        // Initialize the completion map for this material type
        ...
        // ✅ FIX: Loop through files and load progress from API
        for (int i = 0; i < files.length; i++) {
          final file = files[i];

          // ✅ Get progress data from API response for THIS SPECIFIC FILE
          ...
          // Set completion status from API
  ...
  Future<void> _syncMaterialProgress({
    ...
    required int fileIndex, // ⭐ ADDED for logging
    ...
    // ⭐ ADDED LOG
    print("  ➡️ Preparing to sync progress for '${file.name}' [S.No: ${file.sno}, index: $fileIndex]");
    ...
    try {
      ...
      // Added Snackbar for visibility
      if (mounted) {
        CustomSnackbar.showError(context, "Could not save progress for ${file.name}. Check connection.", title: "Sync Failed");
      }
    }
  }

  void _markFileAsCompleted(MaterialFile file, String materialType, int fileIndex) {
    if (mounted && (_fileCompletionStatus[materialType]?[fileIndex] ?? false) == false) {
      print("✔️ Marking '${file.name}' [S.No: ${file.sno}, index: $fileIndex] as completed and syncing with API.");
      setState(() {
        _fileCompletionStatus[materialType]![fileIndex] = true;
        _calculateOverallCompletion();
      });
      _syncMaterialProgress(file: file, materialType: materialType, isCompleted: true, fileIndex: fileIndex);
    }
  }

  void _updateVideoProgress(MaterialFile file, String materialType, int fileIndex, VideoWatchProgress progress) {
    if (mounted) {
      // ⭐ ADDED LOG
      print("🔄 Received video progress update for '${file.name}' [S.No: ${file.sno}, index: $fileIndex]. Progress: ${progress.watchedPercentage.toStringAsFixed(1)}%");

      bool wasCompleted = _fileCompletionStatus[materialType]?[fileIndex] ?? false;
      ...
      _syncMaterialProgress(file: file, materialType: materialType, videoProgress: progress, isCompleted: isNowCompleted, fileIndex: fileIndex);
    }
  }

  void _updatePdfProgress(MaterialFile file, String materialType, int fileIndex, PdfWatchProgress progress) {
    if (!mounted) return;
    print("📄 PDF '${file.name}' [S.No: ${file.sno}, index: $fileIndex] progress: Page ${progress.lastPageViewed}/${progress.totalPages}. Syncing.");
    _syncMaterialProgress(file: file, materialType: materialType, pdfProgress: progress, fileIndex: fileIndex);
  }

  Future<void> _syncChapterProgressOnEntry() async {
    if (_userCode == null || widget.chapter.completionStatus != 'Not Started') return;
    try {
      await AnalyticsService.updateChapterProgress(userCode: _userCode!, subjectId: widget.subjectId, chapterId: widget.chapter.chapterId, completionStatus: 'In Progress', progressPercentage: widget.chapter.progressPercentage);
    } catch (e) { print('⚠️ Failed to update chapter status on entry: $e'); }
  }
  ...
  Future<void> _syncChapterProgressOnExit() async {
    ...
    try {
      await AnalyticsService.updateChapterProgress(userCode: _userCode!, subjectId: widget.subjectId, chapterId: widget.chapter.chapterId, completionStatus: _overallCompletionPercentage >= 100 ? 'Completed' : 'In Progress', progressPercentage: _overallCompletionPercentage, timeSpentMinutes: elapsedMinutes);
    } catch (e) { print('⚠️ Failed to sync chapter progress on exit: $e'); }
  }

  Future<void> _recalculateAndSyncChapterProgress() async {
    if (_userCode == null) return;
    _calculateOverallCompletion();
    final newStatus = _overallCompletionPercentage >= 100 ? 'Completed' : 'In Progress';
    try {
      await AnalyticsService.updateChapterProgress(userCode: _userCode!, subjectId: widget.subjectId, chapterId: widget.chapter.chapterId, completionStatus: newStatus, progressPercentage: _overallCompletionPercentage, timeSpentMinutes: 0);
    } catch(e) { print('⚠️ Failed to sync overall chapter progress after material update: $e'); }
  }
  ...
  void _calculateOverallCompletion() {
    ...
    setState(() {
      _overallCompletionPercentage = totalFiles > 0 ? (completedFiles / totalFiles) * 100 : 0.0;
    });
  }
  ...
  Widget _buildMobileLayout(bool isMobile, bool isDesktop) {
    return Column(
      ...
      children: [
        ...
        else if (_errorMessage != null) _buildErrorState()
        else _buildMaterialsSection(isMobile, isDesktop),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(padding: const EdgeInsets.all(12), decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF3B82F6), Color(0xFF8B5CF6)]), shape: BoxShape.circle), child: const Icon(Iconsax.chart_success, color: Colors.white, size: 22)),
              const SizedBox(width: 14),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Learning Progress', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w800, color: AppTheme.darkText)),
                    const SizedBox(height: 3),
                    Text('$completedFiles of $totalFiles files completed', style: GoogleFonts.inter(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
              Text('${_overallCompletionPercentage.toStringAsFixed(0)}%', style: GoogleFonts.inter(fontSize: 22, fontWeight: FontWeight.w900, color: const Color(0xFF3B82F6))),
            ],
          ),
          const SizedBox(height: 16),
          ClipRRect(
            borderRadius: BorderRadius.circular(10),
            child: LinearProgressIndicator(value: _overallCompletionPercentage / 100, minHeight: 8, backgroundColor: Colors.grey.shade200, valueColor: const AlwaysStoppedAnimation(Color(0xFF3B82F6))),
          ),
          ...
          ..._fileCompletionStatus.entries.map((entry) {
            ...
            if (config == null || entry.value.isEmpty) return const SizedBox.shrink();
  ...
  Widget _buildProgressBreakdownRow({required MaterialTypeConfig config, required int completed, required int total}) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 2, child: Text(config.title, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: AppTheme.darkText))),
          Expanded(flex: 3, child: ClipRRect(borderRadius: BorderRadius.circular(6), child: LinearProgressIndicator(value: percentage, minHeight: 6, backgroundColor: config.color.withOpacity(0.15), valueColor: AlwaysStoppedAnimation(config.color)))),
          const SizedBox(width: 12),
          SizedBox(width: 50, child: Text('$completed/$total', textAlign: TextAlign.right, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.grey.shade700))),
        ],
      ),
    );
  }
  ...
  void _openDocument(MaterialFile file, MaterialTypeConfig config, int index) {
    // ✅ MARK AS COMPLETE IMMEDIATELY when document is clicked
    ...
    if (file.type == 'pdf') {
          ...
          MaterialPageRoute(
            builder: (context) => PdfViewerScreen(
              ...
              onPageChange: (progress) {
                ...
                // ✅ Optional: Mark complete when all pages are viewed
                // if (progress.lastPageViewed >= progress.totalPages && progress.totalPages > 0) {
                //   markFileAsCompleted(file, config.key, index);
                // }
              },
            ),
          ),
      ...
      // For other file types (Word, Excel, etc.), mark complete and open
      launchUrl(Uri.parse(file.fullUrl), mode: LaunchMode.externalApplication);
    }
  }
  ...
  void _openVideo(MaterialFile file, String materialType, int index, VideoWatchProgress? currentProgress) {
    // ⭐ ADDED LOG
    print("📹 Opening video player for '${file.name}' [S.No: ${file.sno}, index: $index]");
    ...
    Navigator.push(context, MaterialPageRoute(builder: (context) => YoutubePlayerScreen(videoId: videoId, title: file.name, subjectColor: widget.subjectColor, initialProgress: currentProgress,
  ...
  Widget _buildChapterInfoCard(bool isMobile) {
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(padding: EdgeInsets.all(isMobile ? 10 : 12), decoration: BoxDecoration(color: widget.subjectColor.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: Icon(Iconsax.book_1, color: widget.subjectColor, size: isMobile ? 20 : 22)),
            SizedBox(width: isMobile ? 12 : 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Chapter ${widget.chapter.chapterOrder}', style: GoogleFonts.inter(fontSize: isMobile ? 11 : 12, color: Colors.grey.shade600, fontWeight: FontWeight.w600)),
              const SizedBox(height: 4),
              Text(widget.chapter.displayChapterName, style: GoogleFonts.inter(fontSize: isMobile ? 16 : 18, fontWeight: FontWeight.w700, color: AppTheme.darkText), maxLines: 2, overflow: TextOverflow.ellipsis)
            ]))
          ]),
          if (widget.chapter.chapterDescription != null) ...[
            SizedBox(height: isMobile ? 12 : 14),
            Text(widget.chapter.chapterDescription!, style: GoogleFonts.inter(fontSize: isMobile ? 13 : 14, color: Colors.grey.shade700, height: 1.5))
          ],
          SizedBox(height: isMobile ? 14 : 16),
          Wrap(spacing: isMobile ? 8 : 10, runSpacing: isMobile ? 8 : 10, children: [
            _buildInfoChip(Iconsax.activity, '${widget.chapter.progressPercentage.toStringAsFixed(0)}%', widget.subjectColor, isMobile),
            _buildInfoChip(Iconsax.clock, '${widget.chapter.timeSpentMinutes} min', widget.subjectColor, isMobile),
  ...
  Widget _buildInfoChip(IconData icon, String label, Color color, bool isMobile) {
    return Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(label, style: GoogleFonts.inter(fontSize: isMobile ? 11 : 12, fontWeight: FontWeight.w600, color: color))
        ])
    );
  }
  ...
  Widget _buildStatusChip(String status, bool isMobile) {
    ...
    IconData icon;
    switch (status) {
      case 'Completed': color = const Color(0xFF10B981); icon = Iconsax.tick_circle; break;
      case 'In Progress': color = const Color(0xFFF59E0B); icon = Iconsax.clock; break;
      default: color = Colors.grey.shade600; icon = Iconsax.document_text;
    }
    return Container(
        padding: EdgeInsets.symmetric(horizontal: isMobile ? 10 : 12, vertical: isMobile ? 6 : 8),
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: isMobile ? 14 : 15, color: color),
          SizedBox(width: isMobile ? 6 : 7),
          Text(status, style: GoogleFonts.inter(fontSize: isMobile ? 11 : 12, fontWeight: FontWeight.w700, color: color))
        ])
    );
  }
  ...
  Widget _buildMaterialsSection(bool isMobile, bool isDesktop) {
    ...
    materials.forEach((key, value) {
      if (value is Map && value['available'] == true && value['files'] != null) {
        ...
        if (config != null) {
          final files = (value['files'] as List).map((f) => MaterialFile.fromJson(f)).toList();
          if (files.isNotEmpty) {
    ...
    return FadeTransition(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
              ...
              child: Row(children: [
                Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(gradient: LinearGradient(colors: [widget.subjectColor.withOpacity(0.15), widget.subjectColor.withOpacity(0.08)]), borderRadius: BorderRadius.circular(12)), child: Icon(Iconsax.folder_2, color: widget.subjectColor, size: 24)),
                const SizedBox(width: 14),
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Study Materials', style: GoogleFonts.inter(fontSize: isMobile ? 18 : 22, fontWeight: FontWeight.w800, color: AppTheme.darkText)),
                  Text('${materialSections.length} material types available', style: GoogleFonts.inter(fontSize: isMobile ? 12 : 13, color: Colors.grey.shade600, fontWeight: FontWeight.w500))
                ])
              ])
          ),
  ...
  Widget _buildMaterialTypeSection(MaterialTypeConfig config, List<MaterialFile> files, bool isMobile) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(color: config.color.withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: config.color.withOpacity(0.3))),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(config.icon, size: 16, color: config.color),
              const SizedBox(width: 8),
              Text(config.title, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700, color: config.color)),
              const SizedBox(width: 8),
              Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: config.color, borderRadius: BorderRadius.circular(10)), child: Text('${files.length}', style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.white)))
            ])
        ),
      ]),
      SizedBox(height: isMobile ? 12 : 16),
      if (config.key == 'Video') _buildVideoGrid(files, config, isMobile) else _buildDocumentList(files, config, isMobile)
    ]);
  }
  ...
  Widget _buildVideoGrid(List<MaterialFile> files, MaterialTypeConfig config, bool isMobile) {
    return GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: isMobile ? 1 : 2, childAspectRatio: isMobile ? 1.4 : 1.3, crossAxisSpacing: 16, mainAxisSpacing: 16),
        ...
        itemBuilder: (context, index) {
          ...
          // ⭐ ADDED LOG
          print("🎨 Building Video Card for '${file.name}' [S.No: ${file.sno}, index: $index]");
  ...
  Widget _buildVideoCard(MaterialFile file, MaterialTypeConfig config, int index, bool isCompleted, VideoWatchProgress? watchProgress, bool isMobile) {
    ...
    final progressText = hasProgress ? '${watchProgress!.watchedPercentage.toStringAsFixed(0)}% watched' : 'Not started';
    final resumeText = hasProgress ? 'Resume from ${_formatDuration(watchProgress!.lastPositionSeconds)}' : 'Start watching';
    return GestureDetector(
        onTap: () => _openVideo(file, config.key, index, watchProgress),
        child: Container(
            decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.white, Colors.grey.shade50], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(16), border: Border.all(color: config.color.withOpacity(0.2)), boxShadow: [BoxShadow(color: config.color.withOpacity(0.12), blurRadius: 16, offset: const Offset(0, 6))]),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Stack(children: [
                ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(16)), child: AspectRatio(aspectRatio: 16 / 9, child: Stack(children: [
                  Image.network(thumbnailUrl, fit: BoxFit.cover, width: double.infinity, errorBuilder: (_, __, ___) => Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [config.color.withOpacity(0.2), config.color.withOpacity(0.1)])), child: Center(child: Icon(Iconsax.video, size: isMobile ? 42 : 54, color: config.color.withOpacity(0.5))))),
                  Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.black.withOpacity(0.2), Colors.transparent, Colors.black.withOpacity(0.4)], begin: Alignment.topCenter, end: Alignment.bottomCenter)))
                ]))),
                Positioned(top: 8, left: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.black.withOpacity(0.6), borderRadius: BorderRadius.circular(6)), child: Text('#${file.sno}', style: GoogleFonts.inter(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700)))),
                Positioned.fill(child: Center(child: Container(padding: EdgeInsets.all(isMobile ? 16 : 20), decoration: BoxDecoration(gradient: LinearGradient(colors: [config.color, config.color.withOpacity(0.8)]), shape: BoxShape.circle, boxShadow: [BoxShadow(color: config.color.withOpacity(0.4), blurRadius: 20, spreadRadius: 2)]), child: Icon(Iconsax.play, size: isMobile ? 28 : 36, color: Colors.white)))),
                if (isCompleted) Positioned(top: 8, right: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF10B981), Color(0xFF059669)]), borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: const Color(0xFF10B981).withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3))]), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Iconsax.tick_circle, size: 12, color: Colors.white), const SizedBox(width: 4), Text('Watched', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w700, color: Colors.white))]))),
                if (hasProgress && (watchProgress?.watchedPercentage ?? 0) < 100) Positioned(bottom: 0, left: 0, right: 0, child: Container(height: 4, color: Colors.black.withOpacity(0.3), child: FractionallySizedBox(alignment: Alignment.centerLeft, widthFactor: watchProgress!.watchedPercentage / 100, child: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [config.color, config.color.withOpacity(0.7)]))))))
              ]),
              Expanded(child: Padding(padding: EdgeInsets.all(isMobile ? 12 : 14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(file.name, style: GoogleFonts.inter(fontSize: isMobile ? 14 : 15, fontWeight: FontWeight.w700, color: AppTheme.darkText), maxLines: 2, overflow: TextOverflow.ellipsis),
                const Spacer(),
                Row(children: [
                  Icon(Iconsax.video, size: 12, color: Colors.grey.shade600),
                  const SizedBox(width: 4),
                  Expanded(child: Text(hasProgress ? resumeText : progressText, style: GoogleFonts.inter(fontSize: 10, color: hasProgress ? config.color : Colors.grey.shade600, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis))
                ])
              ])))
            ])
        )
  ...
  Widget _buildDocumentCard(MaterialFile file, MaterialTypeConfig config, int index, bool isCompleted, bool isMobile) {
    return GestureDetector(
        onTap: () => _openDocument(file, config, index),
        child: Container(
            padding: EdgeInsets.all(isMobile ? 12 : 16),
            decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.white, config.color.withOpacity(0.02)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(14), border: Border.all(color: config.color.withOpacity(0.25)), boxShadow: [BoxShadow(color: config.color.withOpacity(0.08), blurRadius: 10, offset: const Offset(0, 4))]),
            child: Row(children: [
              Container(width: 56, height: 56, decoration: BoxDecoration(gradient: LinearGradient(colors: [config.color, config.color.withOpacity(0.7)]), borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: config.color.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3))]), child: Stack(alignment: Alignment.center, children: [
                Icon(config.icon, color: Colors.white, size: 24),
                Positioned(top: 4, right: 4, child: Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: Colors.black.withOpacity(0.3), shape: BoxShape.circle), child: Text('${file.sno}', style: GoogleFonts.inter(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w700))))
              ])),
              const SizedBox(width: 14),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(file.name, style: GoogleFonts.inter(fontSize: isMobile ? 14 : 15, fontWeight: FontWeight.w700, color: AppTheme.darkText), maxLines: 2, overflow: TextOverflow.ellipsis),
                ...
                Row(children: [
                  Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: config.color.withOpacity(0.1), borderRadius: BorderRadius.circular(4)), child: Text(file.type.toUpperCase(), style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w700, color: config.color))),
                  const SizedBox(width: 8),
                  if (isCompleted) ...[
                    Icon(Iconsax.tick_circle, size: 12, color: const Color(0xFF10B981)),
                    const SizedBox(width: 4),
                    Text('Completed', style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w600, color: const Color(0xFF10B981)))
                  ]
                ])
              ])),
              const SizedBox(width: 12),
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: config.color.withOpacity(0.1), borderRadius: BorderRadius.circular(10)), child: Icon(Iconsax.arrow_right_3, color: config.color, size: 18))
            ])
        )
    );
  }
  ...
  Widget _buildLoadingState() {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      BeautifulLoader(type: LoaderType.pulse, size: 60, color: widget.subjectColor),
      const SizedBox(height: 18),
      Text('Loading materials...', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w600, color: AppTheme.bodyText))
    ]));
  }
  ...
  Widget _buildErrorState() {
    return Center(child: Column(children: [
      ...
      const SizedBox(height: 18),
      Text('Failed to load materials', style: GoogleFonts.inter(fontSize: 17, fontWeight: FontWeight.w700, color: AppTheme.darkText)),
      const SizedBox(height: 10),
      Text(_errorMessage ?? 'Unknown error', style: GoogleFonts.inter(fontSize: 13, color: AppTheme.bodyText.withOpacity(0.7)), textAlign: TextAlign.center),
      const SizedBox(height: 20),
      ElevatedButton.icon(onPressed: _loadMaterials, icon: const Icon(Iconsax.refresh, size: 17), label: const Text('Try Again'), style: ElevatedButton.styleFrom(backgroundColor: widget.subjectColor, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 22, vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(11))))
    ]));
  }
}

class MaterialTypeConfig {
  final IconData icon; final String title; final Color color; final String key;
  MaterialTypeConfig({required this.icon, required this.title, required this.color, required this.key});
}

class VideoWatchProgress {
  final double watchedPercentage; final int lastPositionSeconds; final int totalDurationSeconds;
  VideoWatchProgress({required this.watchedPercentage, required this.lastPositionSeconds, required this.totalDurationSeconds});
}
